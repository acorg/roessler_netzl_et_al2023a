...
data {
  ...
  real<lower=0> sigma_prior_dataset_bias;
...
parameters {
  ...
  array[N_ags, N_sr_groups, N_datasets] real dataset_bias;
...
model {
  ...
  for (i in 1:N_datasets) {
    for (j in 1:N_sr_groups) {
      dataset_bias[,j,i] ~ normal(0, sigma_prior_dataset_bias);
    }
  }
  ...
  for (i in 1:N) {
    ...
    real logtiter = ag_means[ag, sr_group] + 
      ...
      sr_effects[sr] + 
      dataset_bias[ag, sr_group, dataset];
...
generated quantities {
  ...
  for (i in 1:N) {
    ...
    // print(logtiter);
    // print(lower_lims[i]);
    // if (logtiter > lower_lims[i]){
    //   print("In logtiter return if imputed logtiter > lower limit. Otherwise sampled from distribution");
    //   imputed_logtiters[i] = logtiter;
    //
    // } else {
