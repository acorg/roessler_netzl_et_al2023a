...
sr_pretty <- data.frame(
  ...
  'BNT/BNT/BNT', 'Vacc+BA.1', 'Vacc+BA.2', 'Vacc+BA.5', 'BA.1 reinf.', 'BA.2 reinf.', 'BA.2+BA.5 reinf.', 'Vacc+BA.1 reinf.', 'Vacc+BA.2 reinf.', 'AZ/AZ+delta', 'BNT/BNT+delta', "Anc. virus conv.", "D614G conv."),
...
sr_group_gmt_calc_titertools <- function(plotdata, thresh, half_thresh = FALSE) {
  plotdata$below_thresh <- plotdata$titer == paste0("<",thresh)
  ...
  if(!(half_thresh)) {
    ...
    plotdata %>%
      ...
      summarise(
        all_below_thresh = !(FALSE %in% below_thresh),
        gmt = titertools::gmt(titer)) %>%
      mutate(measurement = rownames(gmt),
             logtiter = gmt[,1],
             lower = gmt[,2],
             upper = gmt[,3]) %>%
      select(!gmt) %>%
    ...
    plotdata %>%
      ...
      summarise(all_below_thresh = !(FALSE %in% below_thresh),
        gmt = titertools::gmt(titer)
        ) %>%
      mutate(measurement = rownames(gmt),
             logtiter = gmt[,1],
...
do_titer_plot_fc_label_bayesian_styling <- function(map, facet_col = 3, thresh = 20, fc_label =TRUE, adj_titers = TRUE,
                                   remove_idvl_effects = FALSE,
                                   highlight_samples = NULL, color = "blue", ag_order = NULL, do_ag_color_tile = FALSE) {
  # Set the antigen x axis order
  if(is.null(ag_order)){
    ag_order <- agNames(map)
  } 
  ...
  ymin <- ifelse(do_ag_color_tile, -0.5, 0)
  ymax <- 11.5
  if(remove_idvl_effects){
    plotdata <- adjust_individual_effect(plotdata)
    ymin <- -0.5
...
do_titer_plot_from_long <- function(plotdata, map, facet_col = 3, thresh = 20, fc_label =TRUE, adj_titers = TRUE,
                                   remove_idvl_effects = FALSE,
                                   highlight_samples = NULL, color_var = "sr_group", ymin_supplied = -0.5, ymax_supplied = 11.5,
                                   thresh_plot = 16, titertools_gmt = FALSE, do_ag_color_tile = FALSE) {
  ...
  # Get gmts by serum group
  if(titertools_gmt){
    sr_group_gmt_plotdata <- sr_group_gmt_calc_titertools(plotdata, thresh) 
  } else {
    sr_group_gmt_plotdata <- sr_group_gmt_calc(plotdata, thresh) 
  }
  ...
  sr_group_gmt_plotdata$Data <- sapply(sr_group_gmt_plotdata$sr_group, function(x){
    str_split(x, "-", 2)[[1]][2]
  })
  sr_group_gmt_plotdata$sr_group <- sapply(sr_group_gmt_plotdata$sr_group, function(x){
    str_split(x, "-", 2)[[1]][1]
  })
  
  
  sr_group_gmt_plotdata$sr_group <- factor(sr_group_gmt_plotdata$sr_group, levels = c("Anc. virus conv.", "delta conv.", "BA.1 conv.", "BA.5 conv."))
 # sr_group_gmt_plotdata$Data <- factor(sr_group_gmt_plotdata$Data, levels = c("Hamster-", "Hamster adj. >LOD-","Human-", "Hamster adj. estimate-"))
  ...
  sr_group_gmt_plotdata %>%
    ...
    geom_line(
      aes_string(colour = color_var, group = color_var),
      position = position_dodge(width = 0.1),
      alpha = 1,
      linewidth = 0.75
    ) +
    geom_pointrange(
      aes_string(ymin = "lower", ymax = "upper", colour = color_var, group = color_var),
  ...
  # Annotate region below detectable
  tile_height = 2*(abs(ymin_supplied)+log2(thresh_plot/10))
  gp <- gp +
    annotate(
      "tile",
      x = agNames(map),
...
prep_data_for_plot_gmt <- function(plotdata, thresh, titertools_gmt = TRUE){
