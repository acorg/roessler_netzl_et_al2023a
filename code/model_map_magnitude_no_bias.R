...
model_map <- function(map, 
  ...
  # Build model
  model <- cmdstanr::cmdstan_model("models/titer_comparison_magnitude_no_bias.stan")
...
magnitude_og <- model_map(merged_map, sigma_prior_sr_effect = 1e-3, sigma_prior_dataset_magnitude = 10) 

shinystan::launch_shinystan(magnitude_og$fit)
...
sr_og <- model_map(merged_map, sigma_prior_sr_effect = 10, sigma_prior_dataset_magnitude = 1e-3) 
...
magnitude_both <- model_map(merged_map, sigma_prior_sr_effect = 10, sigma_prior_dataset_magnitude = 10) 

shinystan::launch_shinystan(magnitude_both$fit)
...
saveRDS(
  object = magnitude_both$fit,
  "data/titer_data/merged_map_stan_sampled_both_imputed_fit.rds"
)
...
plot_all_distributions <- function(model, add_muehlemann = FALSE){
  ...
  if(add_muehlemann){
    wrapped <- (plot_list[[1]] + theme(legend.position = "none")) / (plot_list[[2]] + theme(legend.position = "none")) / ((plot_list[[3]] + theme(legend.position = "none")) + plot_magn_diff(model, "dataset_magnitude")) + plot_annotation(tag_levels = 'A') + plot_layout(guides = 'collect')
  } else {
...
plot_magn_diff <- function(models, target_var = "dataset_magnitude|sr_effect|ag_means"){
  ...
  sub_df <- draws_df[, grepl(target_var, colnames(draws_df))]
  colnames(sub_df) <- sapply(colnames(sub_df), function(x) eval(parse(text = x)))
  sub_df <- sub_df %>%
    mutate(difference = Hamster - Human)
  ...
  optim_val %>%
    ...
    mutate(difference = Hamster - Human,
           x = "Hamster - Human organism reactivity") -> optim_val
  
  muehlemann_et_al <- data.frame("Data" = "Muehlemann et al.",
                                 x = "Hamster - Human organism reactivity",
                                 difference = log2(3.1))
  
  optim_val <- plyr::rbind.fill(optim_val, muehlemann_et_al) %>%
    mutate(Data = factor(Data, levels = c("mean", "median", "optimization", "Muehlemann et al.")))
  
  
  sub_df %>%
    mutate(x = "Hamster - Human organism reactivity") %>%
    ggplot(aes(y = difference, x = x)) +
    geom_violin(draw_quantiles =  c(0.25, 0.5, 0.75)) +
    geom_point(data = optim_val, aes(color = Data), position = position_dodge(width = 0.2)) +
    theme_bw() +
    xlab("") +
    ylab("Fold difference") -> pl
...
plots <- plot_all_distributions(magnitude_both, add_muehlemann = TRUE)
